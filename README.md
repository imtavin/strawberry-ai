...
Bem-vindo ao **Strawberry AI**, um sistema completo para **detecção de pragas em morangos**.  
Este repositório funciona como um **umbrella**, centralizando:  
...
O sistema possui três módulos principais: **Backend**, **Frontend** e **IA**.  
A comunicação ocorre via **UDP (stream de vídeo)** e **TCP (comandos)**.

```mermaid
flowchart TB
    Backend[Raspberry Pi - Backend] -->|UDP Stream| Frontend[PC/Cliente - Frontend]
    Frontend -->|TCP Comandos| Backend
    Backend --> IA[Python ML - IA]
...
git clone --recurse-submodules https://github.com/imtavin/strawberry-umbrella.git
cd strawberry-umbrella
